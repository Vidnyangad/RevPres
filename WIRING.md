...
1.  **Pin 1:** Connect one side of the push button to **GPIO 17** (which is Physical Pin 11 on the Raspberry Pi header).
2.  **Pin 2:** Connect the other side of the push button to any **Ground (GND)** pin. For example, Physical Pin 9 or Physical Pin 14.

### Why this works
The application uses the Raspberry Pi's internal pull-up resistor on GPIO 17. This means the pin is naturally held at a HIGH voltage state.
