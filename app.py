...
try:
    ...
    print("Warning: RPi.GPIO module not found. Physical hardware buttons will be disabled.")
...
START_BUTTON_PIN = 17 # BCM GPIO pin for the physical Start button
try:
...
def trigger_start_logic(new_total_time=None):
    """Encapsulated logic to start the presentation from API or hardware button."""
    global current_state, total_time, current_slide_index, remaining_duration
    
    if new_total_time is not None:
        try:
            total_time = float(new_total_time)
        except ValueError:
            pass
...
def gpio_start_callback(channel):
    """Callback function fired when physical Start button is pressed."""
    print("Hardware Start button pressed!")
    trigger_start_logic()

@app.route('/api/start', methods=['POST'])
def start():
    data = request.json or {}
    new_tt = data.get('total_time')
    new_state = trigger_start_logic(new_total_time=new_tt)
...
if __name__ == '__main__':
    ...
    if GPIO_AVAILABLE:
        ...
        # Add event detection for falling edge (button press connecting to Ground)
        # bouncetime helps debounce physical switch noise (300ms)
        GPIO.add_event_detect(START_BUTTON_PIN, GPIO.FALLING, callback=gpio_start_callback, bouncetime=300)
        print(f"Hardware Start button enabled on BCM GPIO {START_BUTTON_PIN}.")
